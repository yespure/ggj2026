...
#if UNITY_EDITOR
        /// <summary>
        ///     Helper method when drawing lists and PropertyFields only draw one item.
        /// </summary>
        public static void PGRegisterChangedCallback(this VisualElement element, Action action)
        {
            switch (element)
            {
                case IntegerField integerField:
                    integerField.RegisterValueChangedCallback(evt => action());
                    break;

                case FloatField floatField:
                    floatField.RegisterValueChangedCallback(evt =>action());
                    break;
                ...
                case Toggle toggleField:
                    toggleField.RegisterValueChangedCallback(evt => action());
                    break;

                case Vector2Field vector2Field:
                    vector2Field.RegisterValueChangedCallback(evt => action());
                    break;

                case Vector3Field vector3Field:
                    vector3Field.RegisterValueChangedCallback(evt => action());
                    break;

                case Vector2IntField vector2IntField:
                    vector2IntField.RegisterValueChangedCallback(evt => action());
                    break;

                case Vector3IntField vector3IntField:
                    vector3IntField.RegisterValueChangedCallback(evt => action());
                    break;

                case ColorField colorField:
                    colorField.RegisterValueChangedCallback(evt => action());
                    break;

                case EnumField enumField:
                    enumField.RegisterValueChangedCallback(evt => action());
                    break;
                
                case Slider slider:
                    slider.RegisterValueChangedCallback(evt => action());
                    break;
                
                case SliderInt sliderInt:
                    sliderInt.RegisterValueChangedCallback(evt => action());
                    break;
                
                case MinMaxSlider minMaxSlider:
                    minMaxSlider.RegisterValueChangedCallback(evt => action());
                    break;
                
                case ObjectField objectField:
                    objectField.RegisterValueChangedCallback(evt => action());
                    break;
                
                case CurveField curveField:
                    curveField.RegisterValueChangedCallback(evt => action());
                    break;
            }
        }
#endif
